class AllRepresentations {
    constructor(projectionNamePrefix) {
        const projectionXY = new TwoDProjection(projectionNamePrefix + '-xy', 'z');
        const projectionXZ = new TwoDProjection(projectionNamePrefix + '-xz', 'y');
        const projectionYZ = new TwoDProjection(projectionNamePrefix + '-yz', 'x');
        ...
        document.getElementById(projectionNamePrefix + '-xy').addEventListener('mouseup', this.updateProjection('xy').bind(this));
        document.getElementById(projectionNamePrefix + '-xz').addEventListener('mouseup', this.updateProjection('xz').bind(this));
        document.getElementById(projectionNamePrefix + '-yz').addEventListener('mouseup', this.updateProjection('yz').bind(this));
    ...
    updateProjection(plane) {
        return () => {
            this.reconstruction3D.drawProjection(plane, this.projections[plane].exportJSON());
        }
    }
    ...
    exportJSON() {
        return {
            xy: this.projections['xy'].exportJSON(),
            xz: this.projections['xz'].exportJSON(),
            yz: this.projections['yz'].exportJSON(),
        }
    }
